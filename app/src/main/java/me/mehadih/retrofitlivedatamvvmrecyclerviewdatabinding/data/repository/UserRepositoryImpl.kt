...
import me.mehadih.retrofitlivedatamvvmrecyclerviewdatabinding.data.local.UserDao
import me.mehadih.retrofitlivedatamvvmrecyclerviewdatabinding.data.local.toDomain
import me.mehadih.retrofitlivedatamvvmrecyclerviewdatabinding.data.remote.UserApi
import me.mehadih.retrofitlivedatamvvmrecyclerviewdatabinding.data.remote.dto.toEntity
...
class UserRepositoryImpl @Inject constructor(
    ...
    override fun observeUsers(): Flow<List<User>> =
        dao.observeUsers().map { entities -> entities.map { it.toDomain() } }

    override fun observeUser(id: Int): Flow<User?> =
        dao.observeUserById(id).map { entity -> entity?.toDomain() }
